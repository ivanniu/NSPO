...
class ScriptArguments:
    ...
    # data parameters
    beta: Optional[float] = field(default=0.001, metadata={"help": "the beta parameter for DPO loss"})
    ...
    lr_warmup_ratio: Optional[float] = field(default=0.03, metadata={"help": "the lr warmup ratio"})
    ...
    prefer_safe: bool = field(default=False, metadata={"help": "whether to prefer safe responses"})
    mix_ratio: float = field(default=0.0, metadata={"help": "mix ratio of safety and helpfulness"})
...
def get_PKU(
    ...
    def return_prompt_and_responses(samples) -> Dict[str, str]:
            ...
            if mix_ratio == 0.0:
                ...
                safe_indices = samples["safer_response_id"]
                helpful_indices = samples['better_response_id']
                np.random.seed(0)
                mask = np.random.random(len(safe_indices)) < mix_ratio
                indices = np.where(mask, safe_indices, helpful_indices).tolist()
                # final_safe_indices = 
            return {
...
if __name__ == '__main__':
    ...
    trainset, evalset = get_PKU(prefer_safe=script_args.prefer_safe,mix_ratio=script_args.mix_ratio)#
